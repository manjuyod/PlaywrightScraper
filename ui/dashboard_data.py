...
WITH active_jobs AS (
    ...
    WHERE status = 'running'
      AND (CAST(:franchise_id AS integer) IS NULL OR grade_scrape_jobs.franchise_id = :franchise_id)
),
recent_jobs AS (
    SELECT
        id,
...
def load_jobs(franchise_id: int | None = None, limit: int = 20) -> list[dict[str, Any]]:
