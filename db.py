...
def load_student_auth_map() -> dict[int, dict]:
    """
...
def get_students(franchise_id: int | None = None, bare: bool = False) -> list[Student | dict]:
    ...
    if not bare:
        students = [Student.create(student) for student in students]
    return students


def add_student(fid: int, student: Student, master_key: bytes):
    # INSERT
    weeklydata = {"2025-08-04": {}, "2025-08-11": {}, "2025-08-18": {}, "2025-08-25": {}, "2025-09-01": {},
    ...
    query = """
            INSERT INTO Student(franchiseid, firstname, lastname, grade, \
...
def filter_group(group: list[Any], key: str | None, value, include=True) -> list[Any]:
    ...
    """
    if key is not None:
        match_check = lambda elem: key in elem.keys() and elem[key] == value
    else:
        match_check = lambda elem: value in elem.values()
