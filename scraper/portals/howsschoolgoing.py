...
from scraper.portals.utils import (
...
class HowsSchoolGoing(PortalEngine):
    portal_key = "howsschoolgoing"
    url_patterns = ("howsschoolgoing",)
    login_config = UniversalLoginConfig(
        username_selector="",
        password_selector="",
        sso_entry_selector='a[href*="google"]',
        google_sso=True,
    )
            ...
            for course in courses:
                ...
                if len(columns) >= 2:
                    ...
                    if grade is not None:
                        parsed[canonicalize_course_title(title)] = grade
