...
class InfiniteCampus(PortalEngine):
    ...
    GRADEBOOK  = "https://campus.ccsd.net/campus/nav-wrapper/parent/portal/parent/grades?appName=clark"      # substring check works fine
    ...
    async def fetch_grades(self) -> dict:
        ...
        await self.page.wait_for_selector("iframe#main-workspace", timeout=30_000)
        ...
        frame = self.page.frame(
            url=lambda u: "/apps/portal/parent/grades" in u
        )
        if not frame:
